...
function renderTripCard(trip) {
  ...
  return `
    <article class="group rounded-3xl border border-slate-200 bg-white p-5 shadow-soft transition hover:-translate-y-0.5 hover:border-primary-200 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-primary-500/30">
      ...
      <div class="mb-4 flex flex-wrap gap-2 text-xs text-slate-500 dark:text-slate-400">
      ...
      ${trip.notes ? `
        <p class="mb-4 text-xs text-slate-500 dark:text-slate-400 line-clamp-2">${escapeHtml(trip.notes)}</p>
      ` : ''}
...
function renderTripList() {
  ...
  container.innerHTML = state.trips.map(renderTripCard).join('');
  refreshIcons();
}
